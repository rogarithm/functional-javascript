...
import {addArrayElements, letters, naiveNth, isIndexed, nth, second, compareLessThanOrEqual, lessOrEqual, existy, truthy, comparator} from './one.js'
const should = chai.should();

describe('1.2.4', function () {
	it('addArrayElements', function () {
		addArrayElements([1, 2]).should.equal(3);
	});
	it('naiveNth', function () {
		naiveNth(letters, 2).should.equal('c');
		//should.Throw(() => naiveNth({}, 1), TypeError);
	});
	...
	it('nth', function () {
		...
		should.Throw(() => nth({},2), Error);
		should.Throw(() => nth(letters, 4000), Error);
		should.Throw(() => nth(letters, 'aaaaa'), Error);
	});
	it('second', function () {
